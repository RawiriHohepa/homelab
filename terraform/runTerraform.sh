...
read CLOUDFLARE_ACCOUNT_ID CLOUDFLARE_ACCESS_KEY CLOUDFLARE_SECRET_KEY < <(echo $(sops decrypt sops/secrets.enc.$ENVIRONMENT.json | jq -r '.cloudflare_backend.account_id, .cloudflare_backend.access_key, .cloudflare_backend.secret_key'))
